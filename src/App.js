          ...
          <div className="row">
            ...
            <div className="col-lg-6 col-xs-12">
              <StackedBarChart
                ...
                toggleVariable="bridge_side"
                query='from(bucket: "my-bucket") |> range(start: 2021-06-06T07:08:22.669468983Z) |> filter(fn: (r) => r["_measurement"] == "tx") |> filter(fn: (r) => r["_field"] == "amount") |> filter(fn: (r) => r["bridge"] == "%bridge%") |> filter(fn: (r) => r["symbol"] =~ /.*USD.*/) |> group(columns: ["symbol", "bridge_side"]) |> sort(columns: ["_time"]) |> map(fn: (r) => ({time: r._time, symbol: r.symbol, bridge_side: r.bridge_side }))'
              />
            </div>
          </div>
        </div>
